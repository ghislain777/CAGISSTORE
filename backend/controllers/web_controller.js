...
const {sendEmailClient, sendEmailUpdateProfile} = require("../services/email/email_service")
...
webController.updateClient = async (req, res) => {
    ...
    //console.log(errors)
    let parametre = req.parametre
if(errors.length != 0) { // Le formunaire n'est pas valide
   ...
   await  Client.update({nom, prenom, quartier, email, motdepasse}, {
    ...
    sendEmailUpdateProfile(req.session.utilisateur)
...
webController.detailCommande = async (req, res) => {
    ...
    parametre.commande = commande
    res.render('detail_commande.html', parametre)
}
...
// on envoi une alerte d'incription au client
sendEmailClient(client,"inscription")
...
webController.getParametre = async (req, res, next) => {
   ...
   const parametresDb = await Parametre.findAll()
    ...
    coutdutransport: parametresDb.find((param, index, array) => param.nom =="LIVRAISON_COUT").valeur,
    delaisdelivraisonenheure: parametresDb.find((param, index, array) => param.nom =="LIVRAISIN_DELAIS_EN_HEURE").valeur,
    delaisdelivraisonenjour: parametresDb.find((param, index, array) => param.nom =="LIVRAISON_DELAIS_EN_JOUR").valeur,
...
if( req.session.utilisateur != undefined || req.session.utilisateur != null) {
    ...
    const livraison = await commandeService.calculFraisDeLivraison(0, parametre.utilisateur.quartier, parametre.entreprise)
    parametre.utilisateur = req.session.utilisateur
...
webController.getPage = (path) => {
    ...
    if(path.includes("/detailcommande")) {
        return {
            nom: "Détails commande #"+path.split("/")[2],
            section: "Profil",
            item: "Détails commande"
    }
