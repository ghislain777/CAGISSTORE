...
export default function EditionVille(props) {
  ...
  const state0 = {
    ...
    progressphoto:0,
     etapes: [
        {
            index: 0,
            label: "Edition du ville"
        },
        {
            index: 1,
            label: "Photos du ville"
        },
...
//* fonctions de gestion de l'atat.
 fonctions.allerAPhotos = (data) => {
  ...
  queries.mutationMiseajour.mutate({...state.ville, ...data})
   ...
   fonctions.uploaderPhoto= (fichier) => {
    if (fichier) {
        //    console.log(fichier);
     mutationUploadMedia.mutate({fichier:fichier, modele:'ville', objet:state.ville, champ:"photo", nom:"Photo ville", handleprogress: handleProgressphoto, obj:{cle: "ville", valeur:state.ville.id}})
        }
...
queries.mutationAjout = useMutation(villeModel.ajoutVille, {
  onSuccess: (data) => {
 ...
 queries.mutationMiseajour = useMutation(villeModel.miseajourVille, {
  onSuccess: (data) => {
     // queryClient.invalidateQueries('Villes')
      setState({
          ...state,
...
const mutationUploadMedia = useMutation(uploaderMedia, {
  onSuccess: (data) => {
    //  const nouveauxmedias = state.ville.Media
...
queries.mutationSuppressionImage = useMutation(villeModel.suppressionImage, {
  onSuccess: (data) => {
