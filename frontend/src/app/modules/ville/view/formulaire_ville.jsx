...
export default function FormulaireVille(props) {
    ...
    nom='description'
    label={`Description`}
    valeur={state.ville?.description }
    control={useform.control}
  ...
  <Grid item>
  {queries.queryPays.isLoading ? (
    <Chargement message='chargement des Pays' />
   ) : (
    <FormSelect
        id='pay'
        name='pay'
        ...
        control={useform.control}
        options={
            queries.queryPays.data['rows']
        }
        error={useform.errors.pay }
        defaultValue = {state.ville?.Pay?.id?? '1'}
        valeur={
            state.ville?.pay?? ''
        }
    ></FormSelect>
