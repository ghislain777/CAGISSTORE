...
import EditionTransporteur from './edition_transporteur';
import IconButton from '@mui/material/IconButton'



const TransporteurView = (props) => {
  ...
  {state.pagestate.mode === 'affichage' ? (
    <Slide in={state.pagestate.affichageTable} direction='right' unmountOnExit={true}>
      <div>
        ...
        {
          queries.queryTransporteurs.isLoading ? <Chargement message='Chargement des données...' type='info'/>  
          : queries.queryTransporteurs.isError? <Chargement message={'Erreur lors du chargement des données::'+queries.queryTransporteurs.error} type='error'/> 
          : <BlockUi tag='div' blocking={false}>
          <MuiThemeProvider theme={getMuiTheme()}>
          <MUIDataTable
            title={state.titre}
            data= {queries.queryTransporteurs.data.rows.map((transporteur)=> {return {transporteur:transporteur}})}
            columns={collonnes}
            options={{...tableOptions, 
              serverSide: true,
              count:queries.queryTransporteurs.data.count,
        ...
        <Card className='w-full overflow-auto' elevation={8} >
          <ConfirmationDialog
            ...
            text={'Confirmez la suppression de ' + state.transporteur?.nom ?? ''}
          />
        </Card>
      </div>
    </Slide>
...
export default TransporteurView
