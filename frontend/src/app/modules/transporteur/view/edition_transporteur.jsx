...
import FormulaireTransporteur from './formulaire_transporteur'
...
export default function EditionTransporteur(props) {
  ...
  const state0 = {
    ...
    progressphoto:0,
     etapes: [
        {
            index: 0,
            label: "Edition du transporteur"
        },
        {
            index: 1,
            label: "Photos du transporteur"
        },
...
//* fonctions de gestion de l'atat.
 fonctions.allerAPhotos = (data) => {
  ...
  if(state.transporteur.id === 0) {
    queries.mutationAjout.mutate(data)
  }
  else {
  queries.mutationMiseajour.mutate({...state.transporteur, ...data})
   ...
   fonctions.uploaderPhoto= (fichier) => {
    if (fichier) {
        //    console.log(fichier);
     mutationUploadMedia.mutate({fichier:fichier, modele:'transporteur', objet:state.transporteur, champ:"photo", nom:"Photo transporteur", handleprogress: handleProgressphoto, obj:{cle: "transporteur", valeur:state.transporteur.id}})
        }
...
queries.mutationAjout = useMutation(transporteurModel.ajoutTransporteur, {
  onSuccess: (data) => {
 ...
 queries.mutationMiseajour = useMutation(transporteurModel.miseajourTransporteur, {
  onSuccess: (data) => {
     // queryClient.invalidateQueries('Transporteurs')
      setState({
          ...state,
...
const mutationUploadMedia = useMutation(uploaderMedia, {
  onSuccess: (data) => {
    //  const nouveauxmedias = state.transporteur.Media
...
queries.mutationSuppressionImage = useMutation(transporteurModel.suppressionImage, {
  onSuccess: (data) => {
        ...
        {state.complet ? (
          ...
          <React.Fragment>
            <Typography sx={{ mt: 2, mb: 1, py: 1 }}>
             {  state.etape ===0 ? <FormulaireTransporteur
              state={state}
                fonctions={fonctions}
                queries={queries}
                useforms={useforms}
