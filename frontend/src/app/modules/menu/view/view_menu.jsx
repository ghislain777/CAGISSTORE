...
import FormulaireMenu from './formulaire_menu';
  ...
  {state.pagestate.mode === 'affichage' ? (
    <Slide in={state.pagestate.affichageTable} direction='right' unmountOnExit={true}>
      <div>
        ...
        {
          queries.queryMenus.isLoading ? <Chargement message='Chargement des données...' type='info'/>  
          : queries.queryMenus.isError? <Chargement message={'Erreur lors du chargement des données::'+queries.queryMenus.error} type='error'/> 
          : <BlockUi tag='div' blocking={false}>
          <MUIDataTable
            title={state.titre}
            data= {queries.queryMenus.data['rows']}
            columns={collonnes}
            options={{...tableOptions, 
              serverSide: true,
              count:queries.queryMenus.data['count'],
        ...
        <Card className='w-full overflow-auto' elevation={8} >
          <ConfirmationDialog
            ...
            text={'Confirmez la suppression de ' + state.menu?.nom ?? ''}
          />
        </Card>
      </div>
    </Slide>
