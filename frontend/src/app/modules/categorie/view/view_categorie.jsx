...
import EditionCategorie from './edition_categorie';
  ...
  {state.pagestate.mode === 'affichage' ? (
    <Slide in={state.pagestate.affichageTable} direction='right' unmountOnExit={true}>
      <div>
        ...
        {
          queries.queryCategories.isLoading ? <Chargement message='Chargement des données...' type='info'/>  
          : queries.queryCategories.isError? <Chargement message={'Erreur lors du chargement des données::'+queries.queryCategories.error} type='error'/> 
          : <BlockUi tag='div' blocking={false}>
          <MuiThemeProvider theme={getMuiTheme()}>
          <MUIDataTable
            title={state.titre}
            data= {queries.queryCategories.data.rows.map((categorie)=> {return {categorie:categorie}})}
            columns={collonnes}
            options={{...tableOptions, 
              serverSide: true,
              count:queries.queryCategories.data.count,
        ...
        <Card className='w-full overflow-auto' elevation={8} >
          <ConfirmationDialog
            ...
            text={'Confirmez la suppression de ' + state.categorie?.nom ?? ''}
          />
        </Card>
      </div>
    </Slide>
