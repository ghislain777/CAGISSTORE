...
import FormulaireQuartier from './formulaire_quartier';



const QuartierView = (props) => {
  ...
  {state.pagestate.mode === 'affichage' ? (
    <Slide in={state.pagestate.affichageTable} direction='right' unmountOnExit={true}>
      <div>
        ...
        {
          queries.queryQuartiers.isLoading ? <Chargement message='Chargement des données...' type='info'/>  
          : queries.queryQuartiers.isError? <Chargement message={'Erreur lors du chargement des données::'+queries.queryQuartiers.error} type='error'/> 
          : <BlockUi tag='div' blocking={false}>
          <MuiThemeProvider theme={getMuiTheme()}>
          <MUIDataTable
            title={state.titre}
            data= {queries.queryQuartiers.data.rows.map((quartier)=> {return {quartier:quartier}})}
            columns={collonnes}
            options={{...tableOptions, 
              serverSide: true,
              count:queries.queryQuartiers.data.count,
        ...
        <Card className='w-full overflow-auto' elevation={8} >
          <ConfirmationDialog
            ...
            text={'Confirmez la suppression de ' + state.quartier?.nom ?? ''}
          />
        </Card>
      </div>
    </Slide>
