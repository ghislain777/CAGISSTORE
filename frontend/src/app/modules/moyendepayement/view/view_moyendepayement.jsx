...
import EditionMoyendepayement from './edition_moyendepayement';
import IconButton from '@mui/material/IconButton'



const MoyendepayementView = (props) => {
  ...
  {state.pagestate.mode === 'affichage' ? (
    <Slide in={state.pagestate.affichageTable} direction='right' unmountOnExit={true}>
      <div>
        ...
        >
        Nouveau moyendepayement
  </Button>
  
        {
          queries.queryMoyendepayements.isLoading ? <Chargement message='Chargement des données...' type='info'/>  
          : queries.queryMoyendepayements.isError? <Chargement message={'Erreur lors du chargement des données::'+queries.queryMoyendepayements.error} type='error'/> 
          : <BlockUi tag='div' blocking={false}>
          <MuiThemeProvider theme={getMuiTheme()}>
          <MUIDataTable
            title={state.titre}
            data= {queries.queryMoyendepayements.data.rows.map((moyendepayement)=> {return {moyendepayement:moyendepayement}})}
            columns={collonnes}
            options={{...tableOptions, 
              serverSide: true,
              count:queries.queryMoyendepayements.data.count,
        ...
        <Card className='w-full overflow-auto' elevation={8} >
          <ConfirmationDialog
            ...
            text={'Confirmez la suppression de ' + state.moyendepayement?.nom ?? ''}
          />
        </Card>
      </div>
    </Slide>
...
export default MoyendepayementView
