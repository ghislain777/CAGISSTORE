...
import FormulaireStatutcommande from './formulaire_statutcommande';



const StatutcommandeView = (props) => {
  ...
  {state.pagestate.mode === 'affichage' ? (
    <Slide in={state.pagestate.affichageTable} direction='right' unmountOnExit={true}>
      <div>
        ...
        {
          queries.queryStatutcommandes.isLoading ? <Chargement message='Chargement des données...' type='info'/>  
          : queries.queryStatutcommandes.isError? <Chargement message={'Erreur lors du chargement des données::'+queries.queryStatutcommandes.error} type='error'/> 
          : <BlockUi tag='div' blocking={false}>
          <MuiThemeProvider theme={getMuiTheme()}>
          <MUIDataTable
            title={state.titre}
            data= {queries.queryStatutcommandes.data.rows.map((statutcommande)=> {return {statutcommande:statutcommande}})}
            columns={collonnes}
            options={{...tableOptions, 
              serverSide: true,
              count:queries.queryStatutcommandes.data.count,
        ...
        <Card className='w-full overflow-auto' elevation={8} >
          <ConfirmationDialog
            ...
            text={'Confirmez la suppression de ' + state.statutcommande?.nom ?? ''}
          />
        </Card>
      </div>
    </Slide>
...
export default StatutcommandeView
