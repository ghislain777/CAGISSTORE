...
import FormulaireCommande from './formulaire_commande';
...
const CommandeView = (props) => {
  ...
  />
        {
          queries.queryCommandes.isLoading ? <Chargement message='Chargement des données...' type='info'/>  
          : queries.queryCommandes.isError? <Chargement message={'Erreur lors du chargement des données::'+queries.queryCommandes.error} type='error'/> 
          : <BlockUi tag='div' blocking={false}>
          <MuiThemeProvider theme={getMuiTheme()}>
          <MUIDataTable
            title={state.titre}
            data= {queries.queryCommandes.data.rows.map((commande)=> {return {commande:commande}})}
            columns={collonnes}
            options={{...tableOptions, 
              serverSide: true,
              count:queries.queryCommandes.data.count,
        ...
        <Card className='w-full overflow-auto' elevation={8} >
          <ConfirmationDialog
            ...
            text={'Confirmez la suppression de ' + state.commande?.nom ?? ''}
          />
        </Card>
      </div>
    </Slide>
