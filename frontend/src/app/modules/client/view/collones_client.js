
import React from 'react'
import { MatxMenu } from '../../../components'
import AddIcon from '@material-ui/icons/Add'
import { Delete, Edit, ArrowRightAlt, ArrowDropDown, Visibility, PostAdd, Check, Add, LocalShipping, EditOutlined, AddAPhotoOutlined } from '@material-ui/icons'
import { makeStyles, useTheme } from '@material-ui/core/styles'
import { Avatar, Box, Button, Checkbox, Divider, Hidden, Icon, IconButton, MenuItem, Typography } from '@material-ui/core';
import { format, parseISO } from 'date-fns'
import { serveur } from '../../../../model/config';
const useStyles = makeStyles(({ palette, ...theme }) => ({
  userMenu: {
    display: 'flex',
    alignItems: 'center',
    cursor: 'pointer',
    borderRadius: 24,
    padding: 4,
    '& span': {
      margin: '0 8px',
      // color: palette.text.secondary
    },
  },
  menuItem: {
    display: 'flex',
    alignItems: 'center',
    minWidth: 185,
  },
}))

export default function getcollonesClient(fonctions) {

  const classes = makeStyles(({ palette, ...theme }) => ({
    userMenu: {
      display: 'flex',
      alignItems: 'center',
      cursor: 'pointer',
      borderRadius: 24,
      padding: 4,
      '& span': {
        margin: '0 8px',
        // color: palette.text.secondary
      },
    },
    menuItem: {
      display: 'flex',
      alignItems: 'center',
      minWidth: 185,
    },
  }))


const getOptionActions = (client) => {
    switch (client.statut) {
      case 'Annulé':
        return [
          {
            id: 1,
            nom: 'Détails',
            action: (client) => handleClickSurAction(client, 'Détails'),
            icon: <Visibility color='primary' />

          },
        ]
      case 'Nouveau':
        return [
          {
            id: 1,
            nom: 'Détails',
            action: (client) => handleClickSurAction(client, 'Détails'),
            icon: <Visibility color='primary' />
          },
          {
            id: 2,
            nom: 'Modifier',
            action: (client) => handleClickSurAction(client, 'Modifier'),
            icon: <EditOutlined color='primary' />
          },
          {
            id: 3,
            nom: 'Supprimer',
            action: (client) => handleClickSurAction(client, 'Supprimer'),
            icon: <Delete color='error' />
          },


        ]

      default:
        return [
          //  {
          //    id: 1,
          //    nom: 'Détails',
          //    action: (client) => handleClickSurAction(client, 'Détails'),
          //    icon: <Visibility color='primary' />
          //  },
           {
             id: 2,
             nom: 'Modifier',
             action: (client) => handleClickSurAction(client, 'Modifier'),
             icon: <EditOutlined color='primary' />
           },
          // {
          //   id: 3,
          //   nom: 'Supprimer',
          //   action: (client) => handleClickSurAction(client, 'Supprimer'),
          //   icon: <Delete color='error' />
          // },
        ]

    }
  }



  const handleClickSurAction = (client, action) => {
   // console.log('Action sur une client::::Action=' + action + ' client :::' + client.id);
   fonctions.handleClickSurAction(client, action)

  }


  return [

    // en commentaire les collones pour les champs complexes et les champs statuts
    //**************collone avec photo ***************************************/ */
    // {
    //   name: 'client',
    //   label: 'Photo',
    //   options: {
    //     filter: false,
    //     sort: false,
    //     display: true,
    //     customBodyRender: (client, tableMeta, updateValue) => {
    //       return <>

    //         <div className='flex items-center'>
    //           <Box onClick={() => fonctions.handleClickSurphoto(client)}>
    //             <Button>
    //               <Avatar
    //                 variant='rounded'
    //                 className='w-48 h-48'
    //                 src={
    //                   serveur +
    //                   '/fichiers/' +
    //                   client.photo
    //                 }
    //               />
    //             </Button>
    //           </Box>
    //           <div className='ml-3'>
    //             <h5 className='my-0 text-15'>{(client.nom ?? '') + (' ') + (client.prenom ?? '')}</h5>
    //             <small className='text-muted'>
    //               {  (client.email ?? '') + '/' +   (client.adresse ?? '')}
    //             </small><br />
    //             <small className='text-muted'>
    //               { (client.telephone ?? '')}
    //             </small>
    //           </div>
    //         </div>
    //       </>
    //     }
    //   }
    // },
    //********************collone de statut ***************************/
    //  {
    //   name: 'client',
    //   label: 'Statut',
    //   options: {
    //     filter: false,
    //     sort: false,
    //     display: true,
    //     customBodyRender: (client, tableMeta, updateValue) => {
    //       return   <div>
    //       <div>
    //         <MatxMenu
    //           menuButton={
    //             <div className={classes.userMenu}>
    //               <Hidden xsDown>
    //                 <Button
    //                   size='small'
    //                   variant='text'
    //                   color='primary'
    //                   endIcon={<ArrowDropDown />}
    //                 >
    //                   <small
    //                     className={'border-radius-4  text-white px-2 py-2px '+ getCouleurStatut(value)}
    //                   >
    //                     {client.statut}
    //                   </small>
    //                 </Button>
    //               </Hidden>
    //             </div>
    //           }
    //         >
    //         {getOptionsStatut(value).map((item) => (
    //                 <MenuItem

    //                 onClick={() =>
    //                   fonctions.handleChangementStatut({
    //                       id: tableMeta.rowData[0],
    //                       statut: item.statut
    //                   })
    //               }
    //                   key={item.id}
    //                   value={item.statut}
    //                 >
    //                   {item.nom}
    //                 </MenuItem>
    //               )
    //               )
    //               }


    //         </MatxMenu>
    //       </div>


    //     </div>
    //   }
    //   }
    // },


    {
        name: 'client',
        label: "ID",
        options: {
          filter: false,
          sort: false,
          display: true,
          customBodyRender: (client, tableMeta, updateValue) => {
            return client.id?.toLocaleString('fr-FR')?? ''
        }
        }
      },
    
    {
        name: 'client',
        label: "Nom",
        options: {
          filter: false,
          sort: false,
          display: true,
          customBodyRender: (client, tableMeta, updateValue) => {
            return <>
                            <div className='flex items-center'>                  
                              <Box onClick={() => fonctions.handleClickSurphoto(client)}>
                                <Button>
                                  <Avatar
                                    variant='rounded'
                                    className='w-48 h-48'
                                    src={
                                      `${client?.Media[0]?.fichier?? ""}`
                                    }
                                  />
                                </Button>
                              </Box>
                              <Box>
                              <Typography style={{whiteSpace:"nowrap"}} variant='subtitle2'>
                        <strong> {client?.nom ?? ""} {client?.prenom ?? ""} </strong> 
                          </Typography> 
                          <Typography style={{whiteSpace:"nowrap"}} variant='caption'>
                          {client?.telephone ?? ""}
                          </Typography> 
                          <br/>
                          <Typography style={{whiteSpace:"nowrap"}} variant='caption'>
                          {client?.email ?? ""}
                          </Typography> 
                            
                          </Box>
                          </div>
                         
                            </>
        }
        }
      },
    
   
      
    {
        name: 'client',
        label: "Description",
        options: {
          filter: false,
          sort: false,
          display: true,
          customBodyRender: (client, tableMeta, updateValue) => {
            return client.description  ?? ''
        }
        }
      },
      {
        name: 'client',
        label: "mot de passe",
        options: {
          filter: false,
          sort: false,
          display: true,
          customBodyRender: (client, tableMeta, updateValue) => {
            return client.motdepasse  ?? ''
        }
        }
      },
    
    {
        name: 'client',
        label: "Créé le",
        options: {
          filter: false,
          sort: false,
          display: true,
          customBodyRender: (client, tableMeta, updateValue) => {
            return client.createdAt == null
            ? ' '
            : format(parseISO(client.createdAt), 'dd/MM/yyyy HH:mm')
            
        }
        }
      },
    
    {
        name: 'client',
        label: "updatedAt",
        options: {
          filter: false,
          sort: false,
          display: 'excluded',
          customBodyRender: (client, tableMeta, updateValue) => {
            return client.updatedAt == null
            ? ' '
            : format(parseISO(client.updatedAt), 'dd/MM/yyyy HH:mm')
            
        }
        }
      },
    
{
    name: 'client',
    label: "Quartier",
    options: {
      filter: false,
      sort: false,
      display: true,
      customBodyRender: (client, tableMeta, updateValue) => {
        return  <>    
                     <Typography style={{whiteSpace:"nowrap"}} variant='subtitle2'>
                      {client.Quartier?.Commune?.nom ?? ""} / {client.Quartier?.nom ?? ""} 
                      </Typography> 
                   
                    <Typography style={{whiteSpace:"nowrap"}} variant='caption'  >
                       {client.Quartier?.Commune?.Ville?.nom?? ""}
                        </Typography> 
                        
                   
                  </>
    }
    }
  },
{
    name: 'client',
    label: 'Photos',
    options: {
      filter: false,
      sort: false,
      display: "excluded",
      customBodyRender: (client, tableMeta, updateValue) => {
        return <>
  <div className='flex items-center'>
  <Box onClick={() => fonctions.handleClickSurphoto(client)}>
    <Button>
      <Avatar
        variant='rounded'
        className='w-48 h-48'
        src={
          serveur +

          client.Media[0]?.fichier
        }
      />
    </Button>
  </Box>
  <Box onClick={() => fonctions.handleAjouterPhoto(client)}>
    <AddAPhotoOutlined />
  </Box>
</div>
<small className='text-muted'>
  {(client.Media.length)}
</small>
</>
}
}
},
  
{
    name: 'client',
    label: 'Actions',
    options: {
      filter: false,
      sort: false,
      display: true,
      customBodyRender: (client, tableMeta, updateValue) => {
        return (

          <div>
            <MatxMenu
              menuButton={
                <div className={classes.userMenu}>
                  <Hidden xsDown>
                    <Button
                      size='small'
                      variant='outlined'
                      color='primary'
                      endIcon={<ArrowDropDown />}
                    >
                      Actions
                    </Button>
                  </Hidden>
                </div>
              }
            >

              {getOptionActions(client).map((item) => (
                  <>
                    <MenuItem

                      onClick={() =>
                        item.action(client)
                      }
                      key={item.id}
                      value={item.statut}
                    >
                      {item.icon}   <span className='ml-5'>{item.nom}</span>
                    </MenuItem>
                    <Divider />
                  </>
                )
                )
              }
            </MatxMenu>
          </div>
        );
      },
    },
  },
]
}

