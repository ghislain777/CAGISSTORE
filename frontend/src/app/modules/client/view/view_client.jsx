...
import EditionClient from './edition_client';
  ...
  {state.pagestate.mode === 'affichage' ? (
    <Slide in={state.pagestate.affichageTable} direction='right' unmountOnExit={true}>
      <div>
        ...
        {
          queries.queryClients.isLoading ? <Chargement message='Chargement des données...' type='info'/>  
          : queries.queryClients.isError? <Chargement message={'Erreur lors du chargement des données::'+queries.queryClients.error} type='error'/> 
          : <BlockUi tag='div' blocking={false}>
          <MuiThemeProvider theme={getMuiTheme()}>
          <MUIDataTable
            title={state.titre}
            data= {queries.queryClients.data.rows.map((client)=> {return {client:client}})}
            columns={collonnes}
            options={{...tableOptions, 
              serverSide: true,
              count:queries.queryClients.data.count,
        ...
        <Card className='w-full overflow-auto' elevation={8} >
          <ConfirmationDialog
            ...
            text={'Confirmez la suppression de ' + state.client?.nom ?? ''}
          />
        </Card>
      </div>
    </Slide>
