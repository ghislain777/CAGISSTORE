...
import FormulaireLignecommande from './formulaire_lignecommande';



const LignecommandeView = (props) => {
  ...
  {state.pagestate.mode === 'affichage' ? (
    <Slide in={state.pagestate.affichageTable} direction='right' unmountOnExit={true}>
      <div>
        ...
        {
          queries.queryLignecommandes.isLoading ? <Chargement message='Chargement des données...' type='info'/>  
          : queries.queryLignecommandes.isError? <Chargement message={'Erreur lors du chargement des données::'+queries.queryLignecommandes.error} type='error'/> 
          : <BlockUi tag='div' blocking={false}>
          <MuiThemeProvider theme={getMuiTheme()}>
          <MUIDataTable
            title={state.titre}
            data= {queries.queryLignecommandes.data.rows.map((lignecommande)=> {return {lignecommande:lignecommande}})}
            columns={collonnes}
            options={{...tableOptions, 
              serverSide: true,
              count:queries.queryLignecommandes.data.count,
        ...
        <Card className='w-full overflow-auto' elevation={8} >
          <ConfirmationDialog
            ...
            text={'Confirmez la suppression de ' + state.lignecommande?.nom ?? ''}
          />
        </Card>
      </div>
    </Slide>
...
export default LignecommandeView
