...
import EditionFournisseur from './edition_fournisseur';
  ...
  {state.pagestate.mode === 'affichage' ? (
    <Slide in={state.pagestate.affichageTable} direction='right' unmountOnExit={true}>
      <div>
        ...
        {
          queries.queryFournisseurs.isLoading ? <Chargement message='Chargement des données...' type='info'/>  
          : queries.queryFournisseurs.isError? <Chargement message={'Erreur lors du chargement des données::'+queries.queryFournisseurs.error} type='error'/> 
          : <BlockUi tag='div' blocking={false}>
          <MuiThemeProvider theme={getMuiTheme()}>
          <MUIDataTable
            title={state.titre}
            data= {queries.queryFournisseurs.data.rows.map((fournisseur)=> {return {fournisseur:fournisseur}})}
            columns={collonnes}
            options={{...tableOptions, 
              serverSide: true,
              count:queries.queryFournisseurs.data.count,
        ...
        <Card className='w-full overflow-auto' elevation={8} >
          <ConfirmationDialog
            ...
            text={'Confirmez la suppression de ' + state.fournisseur?.nom ?? ''}
          />
        </Card>
      </div>
    </Slide>
