...
import FormulaireUtilisateur from './formulaire_utilisateur';
  ...
  {state.pagestate.mode === 'affichage' ? (
    <Slide in={state.pagestate.affichageTable} direction='right' unmountOnExit={true}>
      <div>
        ...
        {
          queries.queryUtilisateurs.isLoading ? <Chargement message='Chargement des données...' type='info'/>  
          : queries.queryUtilisateurs.isError? <Chargement message={'Erreur lors du chargement des données::'+queries.queryUtilisateurs.error} type='error'/> 
          : <BlockUi tag='div' blocking={false}>
          <MuiThemeProvider theme={getMuiTheme()}>
          <MUIDataTable
            title={state.titre}
            data= {queries.queryUtilisateurs.data.rows.map((utilisateur)=> {return {utilisateur:utilisateur}})}
            columns={collonnes}
            options={{...tableOptions, 
              serverSide: true,
              count:queries.queryUtilisateurs.data.count,
        ...
        <Card className='w-full overflow-auto' elevation={8} >
          <ConfirmationDialog
            ...
            text={'Confirmez la suppression de ' + state.utilisateur?.nom ?? ''}
          />
        </Card>
      </div>
    </Slide>
