...
import FormulairePanier from './formulaire_panier'
...
export default function EditionPanier(props) {
  ...
  const state0 = {
    ...
    progressphoto:0,
     etapes: [
        {
            index: 0,
            label: "Edition du panier"
        },
        {
            index: 1,
            label: "Photos du panier"
        },
...
//* fonctions de gestion de l'atat.
 fonctions.allerAPhotos = (data) => {
  ...
  if(state.panier.id === 0) {
    queries.mutationAjout.mutate(data)
  }
  else {
  queries.mutationMiseajour.mutate({...state.panier, ...data})
   ...
   fonctions.uploaderPhoto= (fichier) => {
    if (fichier) {
        //    console.log(fichier);
     mutationUploadMedia.mutate({fichier:fichier, modele:'panier', objet:state.panier, champ:"photo", nom:"Photo panier", handleprogress: handleProgressphoto, obj:{cle: "panier", valeur:state.panier.id}})
        }
...
queries.mutationAjout = useMutation(panierModel.ajoutPanier, {
  onSuccess: (data) => {
 ...
 queries.mutationMiseajour = useMutation(panierModel.miseajourPanier, {
  onSuccess: (data) => {
     // queryClient.invalidateQueries('Paniers')
      setState({
          ...state,
...
const mutationUploadMedia = useMutation(uploaderMedia, {
  onSuccess: (data) => {
    //  const nouveauxmedias = state.panier.Media
...
queries.mutationSuppressionImage = useMutation(panierModel.suppressionImage, {
  onSuccess: (data) => {
        ...
        {state.complet ? (
          ...
          <React.Fragment>
            <Typography sx={{ mt: 2, mb: 1, py: 1 }}>
             {  state.etape ===0 ? <FormulairePanier
              state={state}
                fonctions={fonctions}
                queries={queries}
                useforms={useforms}
