...
import EditionLignepanier from './edition_lignepanier';
import IconButton from '@mui/material/IconButton'



const LignepanierView = (props) => {
  ...
  {state.pagestate.mode === 'affichage' ? (
    <Slide in={state.pagestate.affichageTable} direction='right' unmountOnExit={true}>
      <div>
        ...
        {
          queries.queryLignepaniers.isLoading ? <Chargement message='Chargement des données...' type='info'/>  
          : queries.queryLignepaniers.isError? <Chargement message={'Erreur lors du chargement des données::'+queries.queryLignepaniers.error} type='error'/> 
          : <BlockUi tag='div' blocking={false}>
          <MuiThemeProvider theme={getMuiTheme()}>
          <MUIDataTable
            title={state.titre}
            data= {queries.queryLignepaniers.data.rows.map((lignepanier)=> {return {lignepanier:lignepanier}})}
            columns={collonnes}
            options={{...tableOptions, 
              serverSide: true,
              count:queries.queryLignepaniers.data.count,
        ...
        <Card className='w-full overflow-auto' elevation={8} >
          <ConfirmationDialog
            ...
            text={'Confirmez la suppression de ' + state.lignepanier?.nom ?? ''}
          />
        </Card>
      </div>
    </Slide>
...
export default LignepanierView
