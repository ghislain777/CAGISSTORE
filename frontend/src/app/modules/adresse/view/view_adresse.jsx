...
import FormulaireAdresse from './formulaire_adresse';



const AdresseView = (props) => {
  ...
  {state.pagestate.mode === 'affichage' ? (
    <Slide in={state.pagestate.affichageTable} direction='right' unmountOnExit={true}>
      <div>
        ...
        {
          queries.queryAdresses.isLoading ? <Chargement message='Chargement des données...' type='info'/>  
          : queries.queryAdresses.isError? <Chargement message={'Erreur lors du chargement des données::'+queries.queryAdresses.error} type='error'/> 
          : <BlockUi tag='div' blocking={false}>
          <MuiThemeProvider theme={getMuiTheme()}>
          <MUIDataTable
            title={state.titre}
            data= {queries.queryAdresses.data.rows.map((adresse)=> {return {adresse:adresse}})}
            columns={collonnes}
            options={{...tableOptions, 
              serverSide: true,
              count:queries.queryAdresses.data.count,
        ...
        <Card className='w-full overflow-auto' elevation={8} >
          <ConfirmationDialog
            ...
            text={'Confirmez la suppression de ' + state.adresse?.nom ?? ''}
          />
        </Card>
      </div>
    </Slide>
